...
import { getDescrizioniAttivita } from 'api/centro-costi';
...
function ReactTable({ columns, top}) {
  ...
  const fetchData = useCallback(async ({ pageIndex, pageSize }) => {
    ...
    return await getDescrizioniAttivita(pageIndex, pageSize)
  }, [])
...
const PaginationTable = () => {
  const columns = useMemo(
    () => [
      ...
      {
        Header: 'Associabile a Ordine',
        accessor: 'associabile'
      },
      {
        Header: 'Incl. Riposo "R"',
        accessor: 'include_gg_riposo'
      },
      {
        Header: 'Incl. Trasferte',
        accessor: 'trasferta'
      }
    ],
    []
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <MainCard title="Descrizioni Attività" content={false}>
          <ScrollX>
